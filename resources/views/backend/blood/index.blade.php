    ...
    <table class="table table-bordered table-striped mt-4">
        ...
        <tbody>
            ...
            <tr class="align-middle" id="post-{{ $bloods->id }}">
                ...
                <td> 
                    {!! Str::words($bloods->description, 20) !!}
                </td>
                <td>
                    <div class="form-group ">
                        <!-- Toggle switch (default checked for Active) -->
                        <input type="hidden" name="Status" class="Status" value="0">
                        <input type="checkbox" name="Status" class="Status" data-id="{{$bloods->id}}" data-toggle="toggle" data-on="Active" data-off="Inactive" data-onstyle="success" data-offstyle="danger" value="1" {{ old('Status',$bloods->status) ? 'checked' : '' }}>
                ...
                <td>
                    ...
                    <button id="delete" data-id="{{ $bloods->id }}" class="delete-btn btn btn-danger btn-sm"><i class="fas fa-trash"></i> <b>Delete</b></button>
                </td>
            </tr>
            @endforeach
        </tbody>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                location.reload();
                                // Remove the post element from the DOM (you can select the post by its ID or class)
                                $('#post-' + bloodId).remove(); // Assuming each post has an id like "post-1", "post-2", etc.
                            });
                        },
                        error: function(xhr, status, error) {
                            // Handle any errors
